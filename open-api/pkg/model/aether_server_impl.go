...
func (w *AetherServer) PostConnectivityServices(ctx echo.Context, target Target) error {
    ...
    cs := ConnectivityServices{}

    if err = ctx.Bind(&cs); err != nil {
	log.Error("Error while marshalling",err)
        return ctx.JSON(http.StatusBadRequest,err.Error())
    }
    var response interface{}
    for _ ,entcs := range *cs.ConnectivityService{
        fmt.Println("ConnectivityService %s ",entcs.ConnectivityServiceId)
  	tx := w.DB.Create(&entcs) 
	if tx.Error != nil {
        	log.Infof("Create failed",tx.Error)
...
func (w *AetherServer) GetConnectivityServices(ctx echo.Context, target Target) error {
    ...
    var cscs []*ConnectivityServicesConnectivityService
    tx := w.DB.Find(&cscs)
    if tx.Error != nil {
        log.Infof("Get failed")
        return ctx.JSON(http.StatusOK,tx.Error)
    }
    log.Infof("Count of records : ", len(cscs))
    resource := new(ConnectivityServicesConnectivityServiceList)
    for _ , c1 := range cscs {
        *resource = append(*resource, *c1)
    }
    cs := ConnectivityServices{
        ConnectivityService: resource,
    }
    log.Infof("GetConnectivityServices--",cs)
